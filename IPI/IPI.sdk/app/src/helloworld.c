...
int main()
{
	...
	xil_printf("wwww.AdiuvoEngineering.com IPI Example A53/R5 [Build: %s %s]\r\n", __DATE__, __TIME__);
	...
	if (XST_SUCCESS == Status) {
		xil_printf("Test Completed Pass\r\n");
	} else {
		xil_printf("Test Completed Failed\r\n");
	}
...
static XStatus DoIpiTest(XIpiPsu *InstancePtr)
{
	...
	//xil_printf("Message Content A53:\r\n");
	for (Index = 0; Index < TEST_MSG_LEN; Index++) {
		MsgBuffer[Index] = 0x55555555;
	}
	...
	if (XST_SUCCESS == Status) {
		...
		xil_printf("Received response from R5.\r\n");
		/**
		 * Read the Response buffer
		 */
		XIpiPsu_ReadMessage(InstancePtr, XPAR_XIPIPS_TARGET_PSU_CORTEXR5_0_CH0_MASK, TmpBuffer,
